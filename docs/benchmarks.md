...
5. **用「相对裸 PHP 比例」作稳定主指标——绝对数字不可直接横比**
   kode 是**「分配 / GC 绑定」**的：本机裸 PHP 基线与 Slim 的绝对 req/s 在两次运行间可差 2.4×，而 kode 全栈稳定在同一
   量级——说明其吞吐受**每请求对象分配与 GC** 主导，而非原始 CPU 频率。因此自 v0.8.26 起，压测编排改为**多轮
   （默认 5 轮）+ 比值**：每轮内 kode 与裸 PHP 基线**同机、同状态**测得，再取「kode 吞吐 ÷ 同轮基线」的中位数比例作为
   主指标。机器方差（CPU 调频 / 后台负载 / 缓存温度）在比值中相互抵消，结果可在不同机器、不同时刻间**横向比较**：
   - 实测稳定结论：kode 全栈 `/ping` ≈ 裸 PHP 的 **16.4%**（波动 16.1~17.0%），全栈 `json+DI` ≈ **9.0%**；
     内核（最小中间件）与全栈几乎同量级（16.6% vs 16.4%），印证「瓶颈在每请求分配 / 中间件栈」，而非路由内核。
   - 单进程微基准里，微小的单点分配削减（如去掉一次 Uri 克隆）被 GC 噪声淹没，不构成可测增益；
   - 访问日志已做成**「离路径异步导出」**（v0.8.25，与追踪同范式）：中间件热路径仅一次内存入队，真实格式化 +
     文件写入由响应后 shutdown / 优雅停机钩子批量执行，且保留 `access_log.async=false` 同步退化开关。在**本地文件**
     sink 下该改动吞吐中性（本地 fwrite 廉价、被缓冲，A/B 证实 async 与同步写同机同量级）——印证 kode 并非被日志 I/O
     阻塞；其真实价值在**生产**：把日志侧阻塞（尤其网络 / syslog / 集中式日志等慢 sink）移出请求路径，消除每请求最坏
     情况延迟。
   - **p99 / max 在 CLI 单进程紧循环里高度噪声化**（曾出现某次跑分内核 json+DI p99 3.7ms、max 137ms 的离群，
     重跑即回归 0.07ms 量级）——这是 GC / autoload 抖动，不是框架回归；**以 p50 / 中位数 + 相对比例判断趋势最可靠**。
