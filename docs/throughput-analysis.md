...
### 本版已落地（v0.8.28）：审计增强 —— 脱敏 / 业务事件 / 取证 / 便捷命令

延续 v0.8.27 的「审计近似零成本」立场，在**不增加热路径开销**的前提下补齐审计的合规与开发体验：

- **敏感字段脱敏（安全 / 合规）**：`config/audit.php` 的 `mask_params`（默认 17 个令牌/密码类字段名）
  命中时，HTTP 审计的查询串与 `audit()->event()` 的事件明细中对应值被原地替换为 `***`
  （兼容 `filter[password]=x` 这类嵌套键），**防止令牌、密码等凭据二次泄漏进审计日志**；
  设 `mask_params: []` 即关闭。`mask_body` 默认关（避免读取/改写请求体流），敏感接口可开启。
- **业务 / 安全审计事件 API（便捷开发 + 合规）**：新增 `AuditService::recordEvent($action, $detail, ?$request)`
  与流式别名 `audit()->event('user.login', [...])`，把「谁做了什么业务动作」结构化记入同一审计流
  （登录成功 / 权限变更 / 敏感数据导出 / 配置修改等），与 `record()` 共用离路径异步导出管线，热路径同样仅一次内存入队。
- **取证元数据（安全）**：`record()` 在 `forensic`（默认开）下追加 `user_agent` / `referer`，协助安全溯源；
  `recordEvent` 提供请求时同样补全 `client_ip` / `request_id` / 取证头。
- **`audit:recent` 命令（便捷开发）**：从 `storage/logs/app*.log` 按消息过滤 tail 最近 N 条审计记录
  （支持 `--limit` / `--action`），开发期即时验证「审计是否记录」「敏感字段是否已脱敏」。

**性能保证（回应「企业级中间件加上也不能影响响应太多」）**：上述增强全部发生在审计中间件的热路径上，
...
新增 CSRF 防护以**全局中间件 + 路由按需命中**模式落地（与既有 RateLimit / Feature 同范式）：

- 全局 `CsrfMiddleware` 对每条请求先 `router->match()` + 查 `RouteRegistry::csrfOf()`（spl_object_id
  哈希查表，O(1)），未被 `#[Csrf]` 标记（且非 `auto_apply_unsafe` 命中）的路由**立即早退**，仅一次路由匹配
  + 一次哈希查表，无可感知开销；
- 仅被标记的 cookie-session 路由才触发令牌签发 / 双提交校验；无会话（纯 JWT 接口）即便被标记也安全跳过；
- 令牌存于会话，安全方法（GET/HEAD/OPTIONS）经 `X-CSRF-Token` 响应头引导，非安全方法校验失败返回 419。

**压测验证**：加入全局 `CsrfMiddleware` 后，全栈业务端点（未标记，走早退）审计税 2.92%≈0、框架税 56.48%
（修正基准 ~57%），数字与本版前一致——证明「加上企业级 CSRF 中间件不影响无关路由响应」的设计目标成立。
