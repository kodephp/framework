 ...
 *
 * 引擎由「可注入工厂」提供：默认工厂构造框架中性的 InMemoryBreaker
 * （纯 PHP 状态机，运行时无关，不绑定 kode/fibers）。框架的 Breaker 仅依赖
 * CircuitBreaker 契约，因此引擎可替换（例如未来接入 Redis 分布式熔断），
 * 业务代码无需改动。
 *
...
final class ResilienceServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        $this->container->singleton(Breaker::class, function (): Breaker {
            ...
            // 默认引擎工厂：框架中性 InMemoryBreaker（无 Fiber/协程依赖，通用）。
