...
- 生成时间：2026-08-14T13:00:00+00:00
- 框架版本：kode/framework **0.8.25**
...
|---|---|---:|---:|---:|---:|---:|---:|
| kode · 全栈 (ping) | /ping | 36466 | 0.027 | 0.030 | 0.036 | 0.025 | 0.084 |
| kode · 全栈 (json+DI) | /bench/json | 18180 | 0.053 | 0.060 | 0.084 | 0.047 | 2.183 |
| kode · 内核 (最小中间件) | /ping | 34411 | 0.028 | 0.031 | 0.041 | 0.028 | 0.162 |
| kode · 内核 (json+DI) | /bench/json | 18244 | 0.053 | 0.060 | 0.088 | 0.051 | 0.566 |
| baseline · 裸 PHP (纯逻辑) | (logic) | 204060 | 0.005 | 0.005 | 0.006 | 0.005 | 0.103 |
| Slim 4 · (ping) | /ping | 271948 | 0.004 | 0.004 | 0.004 | 0.003 | 0.073 |
| Slim 4 · (json) | /bench/json | 164598 | 0.006 | 0.006 | 0.008 | 0.006 | 0.109 |
...
|---|---:|
| kode · 全栈 (ping) | 17.87% |
| kode · 全栈 (json+DI) | 8.91% |
| kode · 内核 (最小中间件) | 16.86% |
| kode · 内核 (json+DI) | 8.94% |
| Slim 4 · (ping) | 133.27% |
| Slim 4 · (json) | 80.66% |
...
5. **kode 是「分配 / GC 绑定」的**：跨机器 / 跨时刻的绝对数字波动很大（本机裸 PHP 基线与 Slim 在两次运行间可差 2.4×），
   而 kode 全栈稳定在同一量级——说明其吞吐受每请求对象分配与 GC 主导，而非原始 CPU 频率。因此**减少每请求分配**
   才是继续提响应的真实杠杆。本轮（v0.8.25）已把**访问日志做成「离路径异步导出」**（与追踪同范式）：中间件热路径仅
   做一次内存入队，真实格式化 + 文件写入由响应后的 shutdown / 优雅停机钩子批量执行。在**本地文件** sink 下该改动
   吞吐中性（本地 fwrite 廉价、被缓冲，A/B：async 与同步写同机同量级）——印证 kode 并非被日志 I/O 阻塞。其真实价值在
