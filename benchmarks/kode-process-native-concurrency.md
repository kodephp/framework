...
| 2 | worker 崩溃退出 | `error_log` + `ps` 观察，无 exit/segfault | 排除；worker 静默停 accept |
...
| 5 | macOS 共享监听套接字 + kqueue 惊群 | `defaultReusePort()` 在 macOS 返回 false → 走共享 socket 路径（设计如此） | 与该路径缺陷一致，待定位 |
| 6 | 框架 handler 抛异常打死 worker | Workerman 驱动同路径 157k 健康；错误被 `HttpBridge` try/catch 兜底 | 排除 |

## 4. 怀疑点（供上游定位）

1. **监听器 `onReadable` 回调在首次 accept 后未续挂**：`NativeRuntime::accept()` 在 `runWorker` 中通过 `$loop->onReadable($sock, ...)` 注册一次。若底层 `SelectLoop`/`EventLoop` 的 readable 事件在 `stream_socket_accept()` 消费后未重新触发下次可读回调（事件被「边沿」消费而非「水平」保持），则首个连接 accept 后监听器不再被唤醒 → 后续连接排队在 backlog 直至超时拒绝。
2. **`stream_socket_accept($serverSock, 0, ...)` 与 select 循环的交互**：`timeout=0` 的非阻塞 accept 在 select 报告可读时调用，若 select 已「消耗」该可读事件而 accept 又因某种条件返回 false 并 `return`，可能丢失唤醒。
3. **macOS 无 `SO_REUSEPORT`** 走共享 socket 路径，与 Linux（`reusePort=true`，每 worker 独立 socket）行为不同——该缺陷可能仅 macOS/BSD 复现，Linux 上因独立监听 socket 而正常。**建议上游在 macOS CI 上以 `wrk -c50` 复测**。
...
- **临时规避（用户已拍板）**：压测/生产标定运行时显式用 `KODE_RUNTIME=workerman`（Swoole 驱动另有 §8 回归）；Native 仅作零依赖兜底、不进入性能对比基线。
- **上游修复建议**：核查 `NativeRuntime::accept()` / `LoopFactory` select 实现的 readable 事件续挂语义，并在 macOS 上补充并发压测用例。
